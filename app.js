...
// Camera
async function enableCamera(){ try{ const s=await navigator.mediaDevices.getUserMedia({ video:{ facingMode:{ideal:'user'}, width:{ideal:1280}, height:{ideal:720} } }); els.video.srcObject=s; await els.video.play(); const loop=async()=>{ await hands.send({ image: els.video }); requestAnimationFrame(loop); }; loop(); cameraReady=true; els.status.textContent='Camera ready! Press Play to start'; els.play.disabled=false; els.cameraToggle.textContent='📷 Camera On'; } catch(e){ console.error(e); els.status.textContent='Camera denied or unavailable'; } }
